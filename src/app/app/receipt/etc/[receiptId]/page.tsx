...
import ReceiptEtcGenerator from '@/components/ReceiptEtcGenerator'
...
async function fetchData(receipt_id_param: number) {
    try {
        const { data: fetchedData, error } = await supabase
            .rpc('get_receipt_etc_by_receipt_id', {
                receipt_id_param
            });
        ...
        console.error('Error calling get_receipt_etc_by_receipt_id:', error.message);
    }
}
...
const Receipt = async ({ params }) => {
    ...
    return (
        ...
        <PDFViewer className='h-screen w-full'>
            <ReceiptEtcGenerator data={receiptDetails} />
        </PDFViewer>
